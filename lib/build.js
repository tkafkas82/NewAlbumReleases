...
export async function buildAlbums({ config = {}, onProgress } = {}) {
  ...
  await mapLimit(albums, 4, async (a) => {
    ...
    const [g] = await Promise.all([
      (!a.genre || !a.cover || !a.appleUrl) ? lookupGenre(a.key, a.artist, a.album) : Promise.resolve(null),
      lookupBandcamp(a.key, a.artist, a.album).then((url) => { a.bandcampUrl = url || null; }).catch(() => { a.bandcampUrl = null; }),
    ]);
    if (g) {
      if (!a.genre && g.genre) a.genre = g.genre;
      if (!a.cover && g.cover) a.cover = g.cover;
