// Orchestrate a full build: AOTY releases (+ ratings) + Pitchfork Best New
// Music, merged/de-duplicated, then genre + artwork resolved per album.
// `onProgress` receives the live album array so the UI can render as genres resolve.
...
export async function buildAlbums({ config = {}, onProgress } = {}) {
  ...
  for (const b of bnm) {
    ...
    map.set(key, {
      ...
      aotyUrl: null, pitchforkUrl: b.url,
  ...
  let done = 0;
  await mapLimit(albums, 8, async (a) => {
    ...
    // Persist caches periodically so an interrupted first run resumes instead of
    // starting cold; cache hits make the whole thing near-instant next time.
    if (done % 40 === 0) { await persistGenreCache(); await persistBandcampCache(); }
    await sleep(40); // light stagger to stay under the APIs' rate limits
  });
  ...
  return {
    ...
    updatedMs: Date.now(),
    counts: { total: albums.length, aoty: base.length, bnm: albums.filter((a) => a.bnm).length },
    albums,
  };
}
