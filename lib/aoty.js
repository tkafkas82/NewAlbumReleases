...
import { getText, decode, sleep } from './http.js';
...
export async function getReleases(pages = 4) {
  ...
  for (let p = 1; p <= pages; p++) {
    const url = p === 1 ? `${BASE}/releases/` : `${BASE}/releases/?p=${p}`;
    let list = [];
    try {
      const html = await getText(url, { headers: { Referer: `${BASE}/releases/` } });
      list = parseReleases(html);
    } catch (e) {
      console.error(`[aoty] page ${p} failed: ${e.message}`);
      break;
    }
