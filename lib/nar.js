// Optional source: newalbumreleases.net WordPress feed. The site sits behind an
// aggressive Cloudflare challenge that blocks scripted browsers, so the only way
// in is to reuse a `cf_clearance` cookie captured from YOUR real browser (with
// the exact User-Agent that obtained it). Configure both via the app's settings
// panel. If the cookie is missing/expired, this returns [] and the app falls
// back to AOTY only.
import { getText, decode, UA } from './http.js';
import { tidyGenre } from './genre.js';

const FEED = 'https://newalbumreleases.net/feed/';

// Cloudflare fingerprints the TLS/HTTP2 handshake, so a plain Node fetch is
// rejected (403 challenge) even with a valid cf_clearance cookie. `impit` is a
// native client that presents a real Chrome fingerprint, which lets a fresh
// cookie through. It's an optional native addon: if it can't load (e.g. inside
// the packaged pkg binary), we fall back to plain fetch and NAR just won't get
// past Cloudflare — AOTY/Pitchfork are unaffected either way.
let impitPromise;
function getImpit() {
  if (impitPromise === undefined) {
    impitPromise = import('impit')
      .then((m) => new m.Impit({ browser: 'chrome' }))
      .catch((e) => { console.warn('[nar] impit unavailable, using plain fetch:', e.message); return null; });
  }
  return impitPromise;
}

// Fetch the feed as text. Prefers impit (Chrome TLS impersonation); falls back
// to the plain fetch helper. Returns { text, status } or throws on transport error.
async function fetchFeed(headers) {
  const impit = await getImpit();
  if (impit) {
    const res = await impit.fetch(FEED, { headers });
    return { text: await res.text(), status: res.status };
  }
  return { text: await getText(FEED, { headers, retries: 1 }), status: 200 };
}

export async function getNarReleases({ narCookie, narUA } = {}) {
  if (!narCookie) return { items: [], ok: false, reason: 'no-cookie' };
  // cf_clearance is validated against the UA (and TLS fingerprint) of the
  // browser that solved the challenge, so send a full, self-consistent set of
  // Chrome-style headers derived from that UA to maximise the odds Cloudflare
  // accepts a server-side request. (It can still 403 on TLS fingerprint alone.)
  const ua = narUA || UA;
  const chromeM = (ua.match(/Chrome\/(\d+)/) || [])[1];
  const platform = /Windows/i.test(ua) ? '"Windows"'
    : /Macintosh|Mac OS/i.test(ua) ? '"macOS"'
    : /Android/i.test(ua) ? '"Android"'
    : /iPhone|iPad|iPod/i.test(ua) ? '"iOS"' : '"Linux"';
  const headers = {
    'User-Agent': ua,
    'Cookie': narCookie.includes('=') ? narCookie : `cf_clearance=${narCookie}`,
    'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,application/rss+xml,*/*;q=0.8',
    'Accept-Language': 'en-US,en;q=0.9',
    'Referer': 'https://newalbumreleases.net/',
    'Upgrade-Insecure-Requests': '1',
    'Sec-Fetch-Dest': 'document', 'Sec-Fetch-Mode': 'navigate',
    'Sec-Fetch-Site': 'none', 'Sec-Fetch-User': '?1',
  };
  if (chromeM) {
    headers['sec-ch-ua'] = `"Chromium";v="${chromeM}", "Not(A:Brand";v="24", "Google Chrome";v="${chromeM}"`;
    headers['sec-ch-ua-mobile'] = /Mobile/i.test(ua) ? '?1' : '?0';
    headers['sec-ch-ua-platform'] = platform;
  }
  let xml, status;
  try {
    ({ text: xml, status } = await fetchFeed(headers));
  } catch (e) {
    return { items: [], ok: false, reason: e.message };
  }
  if (status && status >= 400) return { items: [], ok: false, reason: `HTTP ${status}` };
  if (/Just a moment|Checking your browser|Attention Required/i.test(xml) || !/<item>/i.test(xml)) {
    return { items: [], ok: false, reason: 'challenge-or-empty' };
  }
  return { items: parseFeed(xml), ok: true };
}

function tag(block, name) {
  const m = block.match(new RegExp(`<${name}[^>]*>([\\s\\S]*?)</${name}>`, 'i'));
  return m ? decode(m[1].replace(/<!\[CDATA\[|\]\]>/g, '')) : '';
}

function parseFeed(xml) {
  const items = [...xml.matchAll(/<item>([\s\S]*?)<\/item>/gi)].map((m) => m[1]);
  const out = [];
  for (const b of items) {
    const title = tag(b, 'title');
    const split = title.split(/\s+[–—-]\s+/); // "Artist – Album"
    const artist = split.length > 1 ? split[0].trim() : '';
    const album = (split.length > 1 ? split.slice(1).join(' - ') : title).trim();
    if (!album) continue;

    const cats = [...b.matchAll(/<category><!\[CDATA\[([\s\S]*?)\]\]><\/category>/gi)].map((m) => decode(m[1]));
    const genre = pickGenre(cats);
    const content = tag(b, 'content:encoded') || tag(b, 'description');
    const coverM = content.match(/<img[^>]+src="([^"]+)"/i);

    out.push({
      artist,
      album,
      genre,
      cover: coverM ? coverM[1] : null,
      narUrl: tag(b, 'link'),
      releaseText: (tag(b, 'pubDate') || '').replace(/\s+\d{2}:\d{2}:\d{2}.*$/, '').trim(),
      source: 'NAR',
    });
  }
  return out;
}

// Feed categories mix genres with noise ("Music", "Album", format tags). Prefer
// the first category that maps to a known genre bucket.
const NOISE = /^(music|album|albums|lossless|flac|mp3|deluxe|ep|single|va|various)$/i;
function pickGenre(cats) {
  for (const c of cats) {
    if (NOISE.test(c)) continue;
    const t = tidyGenre(c);
    if (t) return t;
  }
  return null;
}
